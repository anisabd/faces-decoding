...
from config import study_path, meg_dir, cal, ctc
...
def run_maxwell_filter(subject_id: int) -> None:
    ...
    info = mne.io.read_info(os.path.join(study_path, "ds000117/derivatives/meg_derivatives", subject, 
                                'ses-meg/meg', f'{subject}_ses-meg_task-facerecognition_run-04_proc-tsss_meg.fif'), verbose=False)
    ...
    for run in range(3, 7):
        ...
        with open( os.path.join(study_path, 'ds000117/derivatives/meg_derivatives', subject, 
                            'ses-meg/meg', f'sub-{subject_id:02d}_ses-meg_task-facerecognition_run-{run:02d}_proc-tsss_log.txt')) as fid:
            for line in fid:
        ...
        raw_sss = mne.io.read_raw_fif(os.path.join(study_path, 'ds000117/derivatives/meg_derivatives', subject, 
                            'ses-meg/meg', f'sub-{subject_id:02d}_ses-meg_task-facerecognition_run-{run:02d}_proc-tsss_meg.fif'), verbose='error')
        ...
        raw_sss.filter(
            ...
            fir_design='firwin')
        raw_sss.notch_filter([50, 100], picks=['mag', 'grad'], filter_length='auto', phase='zero')
...
if __name__ == "__main__":
    ...
    parallel(run_func(subject_id) for subject_id in list(range(3, 4)))
