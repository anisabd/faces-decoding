...
def read_sub_epochs(subject_id: int, freq_band: str = None) -> mne.Epochs:
    ...
    if freq_band is None:
        epochs = mne.read_epochs(
            op.join(meg_dir, subject, f"{subject}-epo.fif"), verbose=False
        )
    else:
        epochs = mne.read_epochs(
            op.join(meg_dir, subject, f"{subject}-{freq_band}-epo.fif"), verbose=False
        )
    return epochs
...
def run_spatiotemp_decoder_single_sub(subject_id:int):
    ...
    for modality in ["face_vs_scrambled", "famous_vs_unfamiliar", "famous_vs_scrambled", "unfamiliar_vs_scrambled"]:
        for freq_band in ["raw", "delta", "theta", "alpha", "beta", "low_gamma", "high_gamma"]:    
            ...
            print(f"  Modality: {modality}, Frequency band: {freq_band}")
            ...
            def clv_spatio_temporal(X, y):
                ...
                return score
            print(f"  Running spatio-temporal decoding analysis for subject {subject}, modality {modality} and frequency band {freq_band}...") 
    ...
    print('Saving all scores to csv file...')
...
def run_spatiotemp_decoder_LOSOCV(subject_ids:list[int]): 
    ...
    for test_subject_id in subject_ids:
        print(f"Starting cross-validation with testing subject {test_subject_id:02d}")
...
run_spatiotemp_decoder_LOSOCV(list(range(1, 16)))
